...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        // REF: https://www.webpages.uidaho.edu/ahmed/ce322/class%20notes/Class-4%20the%20vehicle%20-%20vehicle%20resistance%20%201.pdf
        float c = 0.01*(1 + (x_4/44.73));
        float rolling_res = c*m*10*x_4;  // N.m/s 
        //rolling_res_force = rolling_res_force + dt*rolling_res; // N.m
        rolling_res_force = dt*rolling_res; // N.m
        
        output.vehicle_states.x_5 = x_5 + dt *(u - rolling_res_force) ; //N.m.s
        //output.vehicle_states.x_5 = x_5 + dt *(u) ; //N.m.s
        output.vehicle_states.x_6 = x_6 + (dt * w); // rad
        output.vehicle_states.x_4 = x_4 + dt * (1.0 / m) * (u - rolling_res); // m/s
        //output.vehicle_states.x_4 = x_4 + dt * (1.0 / m) * (output.vehicle_states.x_5 - ga * x_4); // m/s
        //output.vehicle_states.x_4 = x_4 + dt * (1.0 / m) * (u - ga * x_4); // m/s
